...
[[gnu::always_inline]] inline void write_padded(std::string_view content, const format_specs& specs,
                         buffer_appender& out) {
...
struct integerx_formatter {
  ...
  auto format(T val, buffer_appender& out) const {
    ...
    // Fast path: default decimal format without specs
    ...
    // 1. 十进制快速路径 (Fast-path for Base 10)
    if (specs.type == presentation::dec) {
      if constexpr (std::is_signed_v<T>) {
        end_ptr = detail::format_signed_lut_to_buf(val, buf);
      } else {
        end_ptr = detail::format_unsigned_lut_to_buf(val, buf);
    ...
    // 2. 其他进制走 std::to_chars 传统路径
      ...
      // 如果是大写十六进制，直接原地 (In-place) 修改 char 数组，不构造
      // std::string
      if (specs.type == presentation::hex_upper) {
        for (char* p = buf; p != end_ptr; ++p) {
          if (*p >= 'a' && *p <= 'z') *p -= 32;
        }
      }
    }

    // 3. 构造 0 开销的 string_view (零内存分配！)
    std::string_view res_view(buf, static_cast<std::size_t>(end_ptr - buf));
...
// 各种整数类型的特化继承 - 使用优化的 integerx_formatter
template <>
...
struct formatter<std::chrono::duration<Rep, Period>> {
  ...
  auto format(const std::chrono::duration<Rep, Period>& dur,
    ...
    // 格式化数值部分 (不带宽度限制)
    int_fmt.format(dur.count(), temp_out);
